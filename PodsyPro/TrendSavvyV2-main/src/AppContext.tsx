...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const fetchFavorites = useCallback((type: string) => {
    fetch(`http://127.0.0.1:8000/favorites/${type}`)
      .then(r => r.json())
  ...
  const fetchHistory = useCallback((type: string) => {
    fetch(`http://127.0.0.1:8000/history/${type}`)
      .then(r => r.json())
  ...
  const toggleFollow = useCallback(async (type: string, id: string, e?: React.MouseEvent) => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:8000/toggle-follow/${type}/${encodeURIComponent(id)}`, { method: 'POST' });
      const data = await res.json();
