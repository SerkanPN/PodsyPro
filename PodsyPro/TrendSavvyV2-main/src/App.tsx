...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (window.location.pathname.includes('/oauth/callback') && code && state) {
      if (!token) return; // Wait for token to load
      setLoading(true);
      fetch(`http://127.0.0.1:8000/etsy/callback?code=${code}&state=${state}`, {
        method: 'POST',
  ...
  const executeShopScan = async (shopId: string, forceRefresh = false) => {
    ...
    setErrorData(null);
    try {
      const res = await fetch(`http://127.0.0.1:8000/shop/${shopId}?force_refresh=${forceRefresh}`);
  ...
  */
  ...
  const handleSyncAll = async () => {
    setSyncing(true);
    try {
      await fetch(`http://127.0.0.1:8000/sync-all`, { method: 'POST' });
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSyncing(true);
    try {
      const res = await fetch('http://127.0.0.1:8000/import-keywords', {
        method: 'POST',
        body: formData,
      });
